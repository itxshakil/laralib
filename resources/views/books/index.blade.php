...
<div class="container mx-auto px-6">
    ...
    <div class="flex flex-col sm:flex-row flex-wrap flex-stretch gap-4 justify-center">
        ...
        <div class="w-full sm:w-1/3 lg:w-1/4 xl:w-1/5 bg-gray-800 text-gray-100 rounded p-4 shadow flex-grow">
            <a href="{{route('books.show',$book)}}" class="text-2xl font-semibold">{{$book->title}}</a>
            <p class="mt-2 font-semibold">Written By @foreach($book->authors as $author) <a
                    href="{{route('authors.show',$author)}}" class="font-normal">{{$author->name}}@if
                    ($loop->remaining),
